...
ステータス変更またはコメントメンションのSlack通知を使う場合は、SlackのIncoming Webhook URLをWebアプリのプロジェクト設定へ登録します。

1. SlackでIncoming Webhookを発行し、投稿先チャンネルを選びます。
2. Webアプリの「プロジェクト設定」→「Slack通知」へWebhook URLを入力し、通知種別と送信文面を確認して「設定を保存」を押します。
3. `clasp push` 後、Webアプリを開いて初回の再認可を行います。`UrlFetchApp` 追加により、利用者ごとに1回だけ再認可が必要です。

同画面でステータス変更・コメントメンションの個別ON/OFF、各送信文面の編集とデフォルト復元、接続テスト、最終配信状態の確認、連携解除ができます。Webhook URL本体は保存後にブラウザへ返しません。未設定または無効でもタスク保存・コメント保存は成功します。
...
HTML         --> 9,757 lines
JavaScript   --> 6,307 lines

[ TOTAL LINES OF CODE: 16,064 ]
```
